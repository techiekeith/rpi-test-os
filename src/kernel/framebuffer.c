...
void framebuffer_init(bool channel_mode)
{
    ...
    fbinfo.channel_mode = channel_mode;
}
...
uint32_t rgba32(rgb_t color)
{
    return (color.red << 24) | (color.green << 16) | (color.blue << 8) | 0xff;
}

uint32_t bgra32(rgb_t color)
{
    return (color.blue << 24) | (color.green << 16) | (color.red << 8) | 0xff;
}
...
static int framebuffer_allocate()
{
    ...
    {
        debug_printf("WARNING: Allocated size %d != expected size %d! This framebuffer will probably not work.\r\n",
...
static int set_display_dimensions_via_framebuffer_channel(int width, int height, int depth, int char_width, int char_height)
{
    mail_message_t msg;
...
static int set_display_dimensions_via_property_channel(int width, int height, int depth, int char_width, int char_height)
{
    if (fbinfo.buf)
    {
        framebuffer_release();
